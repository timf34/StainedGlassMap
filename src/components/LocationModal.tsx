                ...
                <Grid container spacing={3}>
                    {location.stained_glass_pieces.filter(piece => piece.artist.name === artist).map(piece => (
                        <Grid item xs={6} sm={4} md={3} key={piece.title}>
                            ...
                            <Typography align="center" style={{ marginTop: '0.5rem' }}>{piece.title} ({piece.year_created})</Typography>
                        </Grid>
                    ))}
                </Grid>
                {index < artists.length - 1 && <hr style={{ margin: '2rem 0', border: 'none', borderTop: '1px solid #eee' }} />}
            </div>
        ));
    };
